...
use super::types::{DevinList, Outpost, OutpostDevin, OutpostList, Phase, WatchEvent};
...
impl OutpostsClient {
    ...
    /// List a page of queued sessions for `outpost_id`, optionally resuming from
    /// `cursor`.
    pub async fn list(
        &self,
        outpost_id: &str,
    ...
    /// List *all* queued sessions for `outpost_id`, following pagination.
    ///
    ...
    pub async fn list_all(
        ...
        loop {
            let page = self.list(outpost_id, cursor.as_deref(), params).await?;
    ...
    /// Open a watch stream for `outpost_id` starting from `cursor`.
    ///
    ...
    pub fn watch(
        ...
        let mut req = self
            ...
            .get(self.url("/devins"))
            .bearer_auth(&self.token)
            .query(&[("outpost", outpost_id), ("watch", "true")]);
        if let Some(cursor) = cursor {
            req = req.query(&[("cursor", cursor)]);
        }
    ...
    /// List the outposts visible to this token, following pagination.
    pub async fn list_outposts(&self) -> Result<Vec<Outpost>> {
        ...
        loop {
            ...
            let page: OutpostList = Self::check(req.send().await?).await?.json().await?;
            outposts.extend(page.items);
            if !page.has_next_page {
                return Ok(outposts);
            }
