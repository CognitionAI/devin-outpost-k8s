...
impl MockQueue {
    pub async fn start(token: &str) -> Self {
        ...
        let app = Router::new()
            ...
            .route("/opbeta/outposts", get(pools))
...
async fn list_or_watch(
    ...
    let mut rows: Vec<&Row> = guard
        ...
        .filter(|r| params.get("outpost").is_none_or(|p| &r.pool_id == p))
...
async fn watch_stream(
    ...
    let pool = params.get("outpost").cloned();
    let since = params
        .get("cursor")
    ...
    let items: Vec<serde_json::Value> = pool_ids
        ...
        .map(|pool_id| {
            ...
            let active_claims = guard
                ...
                .count();
            json!({
                "metadata": {"outpost_id": pool_id, "account_id": "acc_mock", "created_at": 0},
