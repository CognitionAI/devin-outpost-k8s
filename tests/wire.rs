...
async fn list_pools_follows_the_paginated_envelope() {
    ...
    let mut pools = client.list_outposts().await.unwrap();
    pools.sort_by(|a, b| a.metadata.outpost_id.cmp(&b.metadata.outpost_id));
    assert_eq!(pools.len(), 2);
    assert_eq!(pools[0].metadata.outpost_id, "pool_a");
